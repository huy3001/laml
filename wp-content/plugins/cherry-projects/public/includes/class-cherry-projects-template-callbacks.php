...
class Cherry_Projects_Template_Callbacks {
	...
	public function get_term_name( $attr = array() ) {
		...
		$html = sprintf( '<h5><a class="link-%3$s" href="%1$s">%2$s</a></h5>', $permalink, $title_cut, (int) $this->term_data->term_id );
	...
	public function get_term_permalink( $attr = array() ) {
		...
		$html = sprintf( '<a class="term-permalink link-%3$s simple-icon" href="%1$s">%2$s</a>',
			$permalink,
			$icon_content,
			(int) $this->term_data->term_id
		);
